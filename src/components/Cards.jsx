...
function Cards({id,name, email, picture, addFriend, list}) {
  ...
  return (
    ...
    <Card className='text-center p-2 h-100' >
      ...
      <Card.Body>
        ...
        <Link to={`/user/${id.value}`}><Button variant="primary" size="sm">More</Button></Link>
        {
          list.some((el)=>id.value == el.id.value)
           ? 
           <Button onClick={handleShow} variant="success" size='sm ms-2'>message 🗨️</Button>
           :
           <Button onClick={()=>addFriend({id, name, email, picture})} variant="success" size='sm ms-2'>👥 Add +</Button>
          }
